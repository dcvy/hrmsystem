...
class EmpController extends Controller
{
    ...
    public function processEmployee(Request $request)
    {
        ...
        $user->email    = str_replace(' ', '_', $request->emp_name) . '@gmail.com';
        ...
        return json_encode(['title' => 'Success', 'message' => 'Employee added successfully. Account created successfully', 'class' => 'modal-header-success']);
    ...
    public function searchEmployee(Request $request)
    {
        ...
        if ($request->button == 'Search') {
            ...
            if ($column == '') {
                ...
                $emps = User::with('employee')->where($request->column, $request->string)->paginate(5);
            } else {
                $emps = User::whereHas('employee', function ($q) use ($column, $string) {
                    $q->whereRaw($column . " like '%" . $string . "%'");
                })->with('employee')->get();
    ...
    {
        $emps = User::with('employee')->paginate(5);
    ...
    {
        $promotions = Promotion::with('employee')->paginate(5);
